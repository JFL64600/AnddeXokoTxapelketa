...
export class TournamentPage implements OnInit {
  ...
  ngOnInit() {
    ...
    */
    this.tournament.Girls = GirlsPlayersJSON;
    this.tournament.Boys = BoysPlayersJSON;
    this.tournament.Groups.push(GirlsGroup1JSON as never);
    this.tournament.Groups.push(GirlsGroup2JSON as never);
    this.tournament.Groups.push(BoysGroup1JSON as never);
    this.tournament.Groups.push(BoysGroup2JSON as never);
    this.tournament.Groups.push(BoysGroup3JSON as never);
    this.tournament.Groups.push(BoysGroup4JSON as never);
    /*
  ...
  public GetPlayerID(group: Group, playerIDInScore: number): string {
    console.log(playerIDInScore - 1);
    return group.Players[playerIDInScore - 1].toString();
  }

  public GetPlayerNameFromID(group: Group, playerIDInScore: number): string {
    const playerID = group.Players[playerIDInScore - 1].toString();
    switch (group.Type) {
      case 0:
